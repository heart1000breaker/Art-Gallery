...
function App() {
  ...
  return (
    <div style={{ padding: isMobile ? '1.25rem' : '2rem', maxWidth: '1200px', margin: '0 auto' }}>
      <header style={{ marginBottom: isMobile ? '4rem' : '8rem', textAlign: 'center' }}>
        ...
        <section style={{ maxWidth: '750px', margin: '0 auto', lineHeight: '1.9', color: '#999', fontSize: isMobile ? '1rem' : '1.1rem', fontWeight: 300 }}>
      ...
      <main style={{ padding: isMobile ? '1rem 0 2rem' : '2rem 0' }}>
        <div
          style={{
            display: 'grid',
            gridTemplateColumns: isMobile ? 'repeat(2, minmax(0, 1fr))' : 'repeat(3, 1fr)',
            gap: isMobile ? '5rem 1.25rem' : '10rem 5rem'
          }}
        >
          {INITIAL_ART.map((piece, index) => (
            <div
              ...
              style={{ aspectRatio: '4/5', cursor: 'pointer', marginTop: isMobile ? '36px' : '60px' }}
            >
              ...
              <div style={{ width: '100%', height: '100%', overflow: 'hidden', position: 'relative' }}>
                ...
                <div className="art-overlay" style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.7)', opacity: 0, transition: 'opacity 0.4s ease', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: isMobile ? '1rem' : '2rem' }}>
                  <span style={{ fontSize: '0.6rem', letterSpacing: '0.3em', textTransform: 'uppercase', color: '#888', marginBottom: '0.5rem' }}>{piece.category}</span>
                  <h3 style={{ fontSize: isMobile ? '0.95rem' : '1.2rem', fontWeight: 300, letterSpacing: '0.1em', marginBottom: '1.5rem' }}>{piece.title}</h3>
                  <div style={{ fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.2em', border: '1px solid white', padding: isMobile ? '0.7rem 1rem' : '0.8rem 1.5rem' }}>View Piece</div>
                </div>
              </div>
              <div style={{ position: 'absolute', bottom: isMobile ? '14px' : '25px', left: isMobile ? '14px' : '25px', display: 'flex', flexDirection: 'column', gap: '4px', zIndex: 6 }}>
                <span style={{ fontSize: isMobile ? '0.56rem' : '0.65rem', textTransform: 'uppercase', letterSpacing: '0.18em', color: '#ccc' }}>{piece.title}</span>
                <span style={{ fontSize: isMobile ? '0.48rem' : '0.55rem', textTransform: 'uppercase', letterSpacing: '0.1em', color: '#666' }}>{piece.category}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {selectedPiece && (
        <div onClick={() => { setSelectedIndex(null); resetView() }} style={{ position: 'fixed', inset: 0, background: 'rgba(5, 5, 5, 0.98)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: isFullView ? '0' : (isMobile ? '1rem' : '4rem'), backdropFilter: 'blur(15px)', cursor: 'zoom-out' }}>
          {!isFullView && (
            <>
              <button onClick={(e) => { e.stopPropagation(); prevPiece() }} aria-label="Previous artwork" style={{ position: 'absolute', left: isMobile ? '0.75rem' : '8rem', top: isMobile ? '40%' : '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: 'white', fontSize: isMobile ? '2.25rem' : '3rem', cursor: 'pointer', opacity: 0.5, transition: 'opacity 0.3s', zIndex: 20 }}>{'<'}</button>
              <button onClick={(e) => { e.stopPropagation(); nextPiece() }} aria-label="Next artwork" style={{ position: 'absolute', right: isMobile ? '0.75rem' : '8rem', top: isMobile ? '40%' : '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: 'white', fontSize: isMobile ? '2.25rem' : '3rem', cursor: 'pointer', opacity: 0.5, transition: 'opacity 0.3s', zIndex: 20 }}>{'>'}</button>
            </>
          )}

          <div onClick={(e) => e.stopPropagation()} style={{ maxWidth: isFullView ? '100vw' : '1400px', width: '100%', maxHeight: isFullView ? '100vh' : (isMobile ? 'calc(100vh - 2rem)' : '90vh'), height: isFullView ? '100vh' : 'auto', display: 'grid', gridTemplateColumns: isFullView ? '1fr' : (isMobile ? '1fr' : '1fr 400px'), gap: isFullView ? '0' : (isMobile ? '1.5rem' : '5rem'), cursor: 'default', background: isFullView ? '#000' : '#0a0a0a', border: isFullView ? 'none' : '1px solid #1a1a1a', padding: isFullView ? '0' : (isMobile ? '1rem' : '3rem'), position: 'relative', transition: 'all 0.4s cubic-bezier(0.4, 0, 0.2, 1)', overflowY: isMobile && !isFullView ? 'auto' : 'visible' }}>
            <div onWheel={handleWheel} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} onClick={handleImageClick} style={{ height: isFullView ? '100vh' : (isMobile ? '46vh' : '70vh'), minHeight: isMobile ? '280px' : undefined, width: '100%', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', cursor: scale > 1 ? (isDragging ? 'grabbing' : 'grab') : 'zoom-in', position: 'relative' }}>
            ...
            {!isFullView && (
              <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', paddingRight: isMobile ? '0' : '1rem', minWidth: 0 }}>
                <span style={{ fontSize: isMobile ? '0.68rem' : '0.75rem', letterSpacing: isMobile ? '0.18em' : '0.3em', textTransform: 'uppercase', color: '#555', marginBottom: '1rem' }}>{selectedPiece.category} // {selectedPiece.series}</span>
                <h2 style={{ fontSize: isMobile ? 'clamp(2rem, 9vw, 2.75rem)' : '3rem', fontWeight: 200, letterSpacing: isMobile ? '0.04em' : '0.1em', marginBottom: '1rem', lineHeight: 1.1, wordBreak: 'break-word' }}>{selectedPiece.title}</h2>
                <p style={{ color: '#aaa', lineHeight: isMobile ? '1.7' : '2', marginBottom: '2rem', fontSize: isMobile ? '1rem' : '1.1rem', fontWeight: 300 }}>{selectedPiece.description}</p>
      ...
      <footer style={{ marginTop: isMobile ? '5rem' : '10rem', paddingBottom: '4rem', textAlign: 'center', color: '#333', fontSize: '0.75rem', letterSpacing: '0.1em' }}>
        {'(c)'} {new Date().getFullYear()} KATIE ROJAS - MIXED-MEDIA GALLERY
      </footer>
    </div>
  )
}
